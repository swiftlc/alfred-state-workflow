...
module.exports = (app) => {
    ...
    app.onAction("copy_to_clipboard", async (context) => {
        ...
        const { _currentSelected, _currentDict } = context.data;

        // 将完整的 JSON 数据格式化后复制到剪切板
        const jsonString = JSON.stringify(_currentSelected, null, 2);
        copyToClipboard(jsonString);
        sendNotification(
            `已复制 ${_currentDict.name}:${_currentSelected.name} 的完整数据`,
            "复制成功",
        );
    });
